...
\widehat{\text{life expectancy}} &= 50.9 + 0.232 \cdot \text{cell phones}
\end{aligned}
```
:::
...
SSE & = `r n-2` \cdot 6.142^2 = 2942.48 
\end{aligned}$$
...
-   Statistical theory tells us that the test statistic $t$ can be
    ...
    -   We had `r n` countries' data, so $n=`n`$
...
-   **For example:** The average life expectancy is `r round(tidy(model1)$estimate[1], 2)` years
    when the number of cell phones per person is 0 (95% CI: `r round(tidy(model1, conf.int = T)$conf.low[1], 2)`, `r round(tidy(model1, conf.int = T)$conf.high[1], 2)`).
