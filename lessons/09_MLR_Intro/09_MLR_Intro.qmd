...
$$\begin{aligned}
\text{LE}  = & \widehat{\beta}_0 + \widehat{\beta}_1 \text{CP} + \widehat{\beta}_2 I(IL = \text{``Lower middle"}) + \widehat{\beta}_3 I(IL = \text{``Upper middle"}) + \widehat{\beta}_2 I(IL = \text{``High"}) \\
\widehat{\text{LE}} = & `r round(mr1_tidy$estimate[1], 3)` + `r round(mr1_tidy$estimate[2], 3)` \ \text{CP}
+ `r round(mr1_tidy$estimate[3], 3)`\ I(IL = \text{``Lower middle"}) + `r round(mr1_tidy$estimate[4], 3)`\ I(IL = \text{``Upper middle"}) + `r round(mr1_tidy$estimate[5], 3)`\ I(IL = \text{``High"})
\end{aligned}$$
