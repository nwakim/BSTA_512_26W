...
-   Let's say many other variables were measured for each country ([see codebook](../gapminder_codebook.qmd))
  ...
  - Basic sanitation: the percentage of people in urban areas with access to basic sanitation services
...
## Visualize relationship between life expectancy, cell phones, and income level
...
```{r}
mr1 <- lm(life_exp ~ cell_phones_100 + basic_sani, 
          data = gapm)
```
...
plot_LE_FS <- ggplot(gapm, aes(x = basic_sani,
                 y = life_exp)) +
  geom_point() +
  geom_smooth() +
  geom_smooth(method = "lm", color = "#F14124") +
...
plot_FLR_FS <- ggplot(gapm, aes(x = basic_sani,
                 y = cell_phones_100)) +
  geom_point() +
  geom_smooth() +
  geom_smooth(method = "lm", color = "#F14124") +
...
mr1 <- gapm %>% 
  lm(formula = life_exp ~ cell_phones_100 + basic_sani)
tidy(mr1, conf.int=T) %>% gt() %>% tab_options(table.font.size = 35) %>% fmt_number(decimals = 3)
```
...
x_grid <- seq(from = min(gapm$cell_phones_100, na.rm = TRUE), 
              ...
              length = 100)
y_grid <- seq(from = min(gapm$basic_sani, na.rm = TRUE), 
              to = max(gapm$basic_sani, na.rm = TRUE), 
              length = 200)
