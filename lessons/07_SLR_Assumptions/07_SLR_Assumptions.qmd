...
-   The relationship between the variables is linear (a straight line):
    -   The mean value of $Y$ given $X$ (aka $\widehat{Y}|X$, $\mu_{y|x}$ or $E[Y|X]$) is a
...
norm_qq = ggplot(normal, aes(sample = x)) + 
  stat_qq() + 
  stat_qq_line() + 
  theme(labs(x = "Theoretical quantiles", y = "Data"))
unif_qq = ggplot(uniform, aes(sample = x)) + 
  stat_qq() + 
  stat_qq_line() + 
  theme(labs(x = "Theoretical quantiles", y = "Data"))
t_qq = ggplot(t, aes(sample = x)) + 
  stat_qq() + 
  stat_qq_line() +
  theme(labs(x = "Theoretical quantiles", y = "Data"))
skewed_qq = ggplot(skewed, aes(sample = x)) + 
...
::: thm-cont
Fail to reject the null. Data are not from a normal distribution.
:::
:::
:::
:::
...
library(ggfortify)
autoplot(model1, size = 1) + theme(text=element_text(size=14))
```
