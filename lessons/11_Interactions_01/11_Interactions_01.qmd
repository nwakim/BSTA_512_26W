...
## A fun example of an interaction model
...
-   Questions of interest: Is the effect of female literacy rate on life expectancy differ depending on income level?
    ...
    -   "effect of female literacy rate" differing = different slopes between CP and LE depending on the income group
...
\widehat{LE} = & 54.85 + 0.156 \cdot CP - 16.65 \cdot I(\text{high income}) + 0.228 \cdot CP \cdot I(\text{high income})
\end{aligned}$$
...
\widehat{LE} = & 54.85 + 0.156 \cdot CP - 16.65 \cdot 0 + \\
& 0.228 \cdot CP \cdot 0 \\ 
\widehat{LE} = & 54.85 + 0.156 \cdot CP\\ 
\end{aligned}$$
:::
:::::
::::::
...
\widehat{LE} = & 54.85 + 0.156 \cdot CP - 16.65 \cdot 1 + \\ & 0.228 \cdot CP \cdot 1 \\ 
\widehat{LE} = & (54.85 - 16.65 \cdot 1) + \\ & (0.156 \cdot CP + 0.228 \cdot CP \cdot 1) \\ 
\widehat{LE} = & (54.85 - 16.65) + (0.156 + 0.228) \cdot CP\\ 
\widehat{LE} = & `r 54.85 - 16.65` + `r 0.156 + 0.228` \cdot CP\\ 
\end{aligned}$$
:::
:::::
::::::
...
-   Intercept of `r 54.85 - 16.65` is misleading because
    ...
    -   There are no high income countries with CP less than \~70%
...
-   Centering female literacy rate: $$ CP^c = CP - \overline{CP}$$
-   Centering in R:

```{r}
#| echo: true 
gapm = gapm %>% 
  mutate(CP_c = cell_phones_100 - median(cell_phones_100))
```
...
-   Now all intercept values (in each respective world region) will be the mean life expectancy when female literacy rate is `r round(mean_CP, 2)`%
...
```{r}
#| echo: true
m_int_inc_red = lm(life_exp ~ CP_c + income_level_2, 
                   data = gapm)
m_int_inc_full = lm(life_exp ~ CP_c + income_level_2 +
                  CP_c*income_level_2, data = gapm)
```
...
$$\begin{aligned}LE = &\beta_0 + \beta_1 CP^c + \beta_2 I(\text{Americas}) + \beta_3 I(\text{Asia}) + \beta_4 I(\text{Europe}) + \\ & \beta_5 CP^c \cdot I(\text{Americas}) + \beta_6 CP^c \cdot I(\text{Asia})+ \beta_7 CP^c \cdot I(\text{Europe})+ \epsilon \end{aligned}$$

-   $LE$ as life expectancy
-   $CP^c$ as centered female literacy rate (continuous variable)
...
$$\begin{aligned} 
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP + \widehat\beta_2 I(\text{Americas}) + \widehat\beta_3 I(\text{Asia}) + \widehat\beta_4 I(\text{Europe}) + \\ & \widehat\beta_5 CP \cdot I(\text{Americas}) + \widehat\beta_6 CP \cdot I(\text{Asia})+ \widehat\beta_7 CP \cdot I(\text{Europe}) \\
\widehat{LE} = & `r round(m_int_wr_fit$estimate[1], 3)` + `r round(m_int_wr_fit$estimate[2], 3)` \cdot CP + `r round(m_int_wr_fit$estimate[3], 3)` \cdot I(\text{Americas}) + `r round(m_int_wr_fit$estimate[4], 3)` \cdot I(\text{Asia}) + `r round(m_int_wr_fit$estimate[5], 3)` \cdot I(\text{Europe}) + \\ & `r round(m_int_wr_fit$estimate[6], 3)`  \cdot CP \cdot I(\text{Americas}) + `r round(m_int_wr_fit$estimate[7], 3)` \cdot CP \cdot I(\text{Asia}) `r round(m_int_wr_fit$estimate[8], 3)`    \cdot CP \cdot I(\text{Europe})
\end{aligned}$$
...
::: proof-cont
$$\begin{aligned} 
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP + \\ & \widehat\beta_2 \cdot 0 + \widehat\beta_3 \cdot 0 + \\ & \widehat\beta_4 \cdot 0 + \widehat\beta_5 CP \cdot 0 + \\ & \widehat\beta_6 CP \cdot 0+ \widehat\beta_7 CP \cdot 0 \\
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP\\
\end{aligned}$$
:::
:::::
...
::: fact-cont
$$\begin{aligned} 
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP + \\ & \widehat\beta_2 \cdot 1 + \widehat\beta_3 \cdot 0 + \\ & \widehat\beta_4 \cdot 0 + \widehat\beta_5 CP \cdot 1 + \\ & \widehat\beta_6 CP \cdot 0+ \widehat\beta_7 CP \cdot 0 \\
\widehat{LE} = &\big(\widehat\beta_0+\widehat\beta_2\big) + \\ &\big(\widehat\beta_1 + \widehat\beta_5\big)CP \\
\end{aligned}$$
:::
:::::
::::::
...
::: prop-cont
$$\begin{aligned} 
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP + \\ & \widehat\beta_2 \cdot 0 + \widehat\beta_3 \cdot 1 + \\ & \widehat\beta_4 \cdot 0 + \widehat\beta_5 CP \cdot 0 + \\ & \widehat\beta_6 CP \cdot 1+ \widehat\beta_7 CP \cdot 0 \\
\widehat{LE} = &\big(\widehat\beta_0+\widehat\beta_3\big) + \\ &\big(\widehat\beta_1 + \widehat\beta_6\big)CP \\
\end{aligned}$$
:::
:::::
::::::
...
::: def-cont
$$\begin{aligned} 
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP + \\ & \widehat\beta_2 \cdot 0 + \widehat\beta_3 \cdot 0 + \\ & \widehat\beta_4 \cdot 1 + \widehat\beta_5 CP \cdot 0 + \\ & \widehat\beta_6 CP \cdot 0+ \widehat\beta_7 CP \cdot 1 \\
\widehat{LE} = &\big(\widehat\beta_0+\widehat\beta_4\big) + \\ & \big(\widehat\beta_1 + \widehat\beta_7\big)CP \\
\end{aligned}$$
:::
:::::
::::::
...
$$ \begin{aligned}
\widehat{LE} = &\widehat\beta_0 + \widehat\beta_1 CP + \widehat\beta_2 I(\text{Americas}) + \widehat\beta_3 I(\text{Asia}) + \widehat\beta_4 I(\text{Europe}) + \\ & \widehat\beta_5 CP \cdot I(\text{Americas}) + \widehat\beta_6 CP \cdot I(\text{Asia})+ \widehat\beta_7 CP \cdot I(\text{Europe}) \\
\widehat{LE} = & \bigg[\widehat\beta_0 + \widehat\beta_2 I(\text{Americas}) + \widehat\beta_3 I(\text{Asia}) + \widehat\beta_4 I(\text{Europe})\bigg] + \\ &\underbrace{\bigg[\widehat\beta_1 +  \widehat\beta_5 \cdot I(\text{Americas}) + \widehat\beta_6 \cdot I(\text{Asia})+ \widehat\beta_7 \cdot I(\text{Europe}) \bigg]}_\text{CP's effect} CP \\
\end{aligned}$$
...
$$\begin{aligned}LE = &\beta_0 + \beta_1 CP + \beta_2 I(\text{Americas}) + \beta_3 I(\text{Asia}) + \beta_4 I(\text{Europe}) + \\ & \beta_5 CP \cdot I(\text{Americas}) + \beta_6 CP \cdot I(\text{Asia})+ \beta_7 CP \cdot I(\text{Europe})+ \epsilon \end{aligned}$$
...
::: proof-cont
$$\begin{aligned}LE = &\beta_0 + \beta_1 CP + \beta_2 I(\text{Americas}) + \\ & \beta_3 I(\text{Asia}) + \beta_4 I(\text{Europe}) + \epsilon \end{aligned}$$
:::
:::::
::::::
...
```{r}
#| echo: true
m_int_wr_red = lm(life_exp ~ CP_c + freedom_status, 
                   data = gapm)
m_int_wr_full = lm(life_exp ~ CP_c + freedom_status+
                  CP_c*freedom_status, data = gapm)
```
...
# newdata <- data.frame(freedom_status = c("Africa", "Americas", "Asia", "Europe"), 
#                       CP_c = c()) 
...
-   World region is NOT an effect measure modifier of CP on LE
