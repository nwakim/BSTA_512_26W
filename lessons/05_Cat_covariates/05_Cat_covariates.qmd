...
gapm = gapm %>%
  mutate(fs_order = case_when(freedom_status == "F" ~ 3,
                                freedom_status == "PF" ~ 2,
                                freedom_status == "NF" ~ 1), 
         freedom_status = fct_relevel(freedom_status, "NF"))
```
...
-   Gapminder measures freedom status: This is a descriptive text of the real-world rights and freedoms enjoyed by individuals. It is determined by the freedom rating which is the average of poltical rights and civil liberties ratings.
...
-   Note: I am calling the expected life expectancy $\widehat{LE}$
    -   Previously, I have FSitten $\widehat{\text{life expectancy}}$
...
-   We want to create a function that can map each freedom status to life expectancy
    ...
    -   If not free: $\widehat{LE} = `r round(means_LE$mean[2], 2)`$ years

    -   If partly free: $\widehat{LE} = `r round(means_LE$mean[3], 2)`$ years

    -   If free: $\widehat{LE} = `r round(means_LE$mean[1], 2)`$ years
...
::: {.column width="50%"}
$$\begin{aligned}
\widehat{\textrm{LE}} = & `r round(means_LE$mean[2], 2)` \cdot I(\text{not free}) + `r round(means_LE$mean[3], 2)` \cdot I(\text{partly free}) + \\ &`r round(means_LE$mean[1], 2)` \cdot I(\text{free})
\end{aligned}$$
...
$$\begin{aligned}
\widehat{\textrm{LE}} = & `r round(means_LE$mean[2], 2)` + `r round(means_LE$mean[3]-means_LE$mean[2], 2)` \cdot I(\text{partly free}) + \\ &`r round(means_LE$mean[1]-means_LE$mean[2], 2)` \cdot I(\text{free}) \\
...
$$\widehat{\textrm{LE}} = `r round(tidy_mod$estimate[1], 2)` + `r round(tidy_mod$estimate[3], 2)` \cdot I(\text{PF}) + `r round(tidy_mod$estimate[2], 2)` \cdot I(\text{F})$$
...
$$\widehat{\textrm{LE}} = `r round(mean_PF,2)` `r round(coeff_PF[1],2)` \cdot I(\text{NF}) + `r round(coeff_PF[2],2)` \cdot I(\text{F})$$
...
$$\begin{aligned}\widehat{\textrm{LE}} &=  \widehat\beta_0 + \widehat\beta_1 \cdot I(\text{NF}) +  \widehat\beta_2 \cdot I(\text{F}) + \\ \widehat{\textrm{LE}} &= `r round(mean_PF,2)` `r round(coeff_PF[1],2)` \cdot I(\text{NF}) + `r round(coeff_PF[2],2)` \cdot I(\text{F}) \end{aligned}$$
...
#| echo: true
gapm2 = gapm2 %>%
 mutate(income_level_4 = factor(income_level_4, 
            ordered = T, 
            levels = c("Low income", 
            "Lower middle income", 
            "Upper middle income", 
...
#| eval: false
#| echo: true
ggplot(gapm, aes(x = income_level_4, y = life_exp)) +
  geom_jitter(size = 1, alpha = .6, width = 0.2) +
  stat_summary(fun = mean, geom = "point", size = 8, shape = 18) +
  labs(x = "Income levels", 
       y = "Country life expectancy (years)",
...
#| echo: true
gapm2 = gapm2 %>%
  mutate(income_num = as.numeric(income_level_4))
str(gapm$income_num)
```
