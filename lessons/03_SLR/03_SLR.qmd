...
::: heq
$$\widehat{\text{life expectancy}} =  `r tidy_model1$estimate[1] %>% round(digits=2)` + `r tidy_model1$estimate[2] %>% round(digits=3)`\cdot\text{cell phones}$$
:::
...
-   [You can access my codebook here](lessons/gapminder_codebook.qmd)
...
-   [**Cell phones per 100 people**](https://data.worldbank.org/indicator/IT.CEL.SETS.P2) is the number of cell phone subscriptions per 100 people in a given population, indicating the level of mobile phone use and accessibility.
...
-   What is the expected life expectancy for a country with a
    specified number of cell phones per 100 people?    
:::
:::
:::
:::
...
-   **Intercept ($\hat{\beta}_0$)**
    -   The expected outcome for the $Y$-variable when the $X$-variable (if continuous)
        is 0
    -   **Example:** The expected/average life expectancy is 60.4 years
        for a country with 0 cell phones per 100 people.
-   **Slope ($\hat{\beta}_1$)**
