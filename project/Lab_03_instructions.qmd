...
model_temp <- iat_2024_comp_factor %>% 
  lm(formulat = iat_score ~ important_f + pol_id_f)
...
```{txt}
`r round(model_temp_tidy$estimate[2], 2)` 
(for the estimate of important_001) and 
`r round(model_temp_tidy$conf.low[2], 2)` and `r round(model_temp_tidy$conf.high[2], 2)` 
(for the confidence intervals of important_001).
```
